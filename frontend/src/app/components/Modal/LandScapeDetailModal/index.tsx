...
const LandscapeDetailModal = forwardRef(
  (
    ...
    const [landscapeDetail, setLandscapeDetail] =
      useState<ILandscapeDetailCardProps | null>(null);

    const getLandscapeDetail = (id: string) => {
      // TODO: 串取景點詳細資料API
      console.log("Fetching landscape detail ID:", id);
    ...
    useImperativeHandle(ref, () => ({
      ...
      toggle: (id: string) => {
        console.log({ id });
        if (open) {
          setLandscapeDetail(null);
        } else {
          getLandscapeDetail(id);
        }
        setOpen((prev) => !prev);
      },
    }));
    return (
      <DialogRoot open={open} onOpenChange={setOpen}>
        <DialogContent className="min-w-[80vw] pb-0" headerWithClose>
          <DialogHeader title={landscapeDetail?.title || ""} />
          <DialogBody className="relative overflow-hidden px-0">
            ...
            >
              {landscapeDetail && (
                <ViewLandscapeDetailCard {...landscapeDetail} />
              )}
              <iframe
                width="100%"
                height="100%"
