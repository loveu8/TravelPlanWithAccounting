...
public class MapService {
  ...
  public MapService(
      HttpClient httpClient, GoogleApiConfig googleApiConfig, ObjectMapper objectMapper) {
  ...
  private JsonNode executeHttpRequest(String apiType, HttpRequest request) {
    try {
      ...
      if (response.statusCode() >= 200 && response.statusCode() < 300) {
        ...
        log.error(
            "Error calling {} API: Status: {}, Response: {}",
            apiType,
            response.statusCode(),
            response.body());
  ...
  /**
   * 4. Place Details (Places API v1) 本地端為 POST 請求，但 Google API 實際仍是 GET 請求，參數透過 URL Query 和 Header
   * 傳遞。
   *
  ...
  public JsonNode getPlaceDetails(PlaceDetailRequestPost request) {
    UriComponentsBuilder uriBuilder =
        UriComponentsBuilder.fromUriString(
            googleApiConfig.getPlacesApiV1BaseUrl() + "places/" + request.getPlaceId());
    if (request.getLanguageCode() != null)
      uriBuilder.queryParam("languageCode", request.getLanguageCode());
    if (request.getRegionCode() != null)
      uriBuilder.queryParam("regionCode", request.getRegionCode());
  ...
  /**
   * 5. Geocoding (Maps Geocoding API) 本地端為 POST 請求，但 Google API 實際仍是 GET 請求，參數透過 URL Query 傳遞。
   *
  ...
  public JsonNode geocode(GeocodingRequest request) {
    UriComponentsBuilder uriBuilder =
        UriComponentsBuilder.fromUriString(googleApiConfig.getMapsApiBaseUrl() + "geocode/json");
    uriBuilder.queryParam("key", googleApiConfig.getGoogleApiKey());

    String queryParamValue = null;
    if (request.getAddress() != null && !request.getAddress().isEmpty()) {
      uriBuilder.queryParam(
          "address", URLEncoder.encode(request.getAddress(), StandardCharsets.UTF_8));
      ...
      throw new IllegalArgumentException(
          "Either address or latLng must be provided for Geocoding API.");
    }
  ...
  /**
   * 6. Directions (Maps Directions API) 本地端為 POST 請求，但 Google API 實際仍是 GET 請求，參數透過 URL Query 傳遞。
   *
  ...
  public JsonNode getDirections(DirectionsRequest request) {
    UriComponentsBuilder uriBuilder =
        UriComponentsBuilder.fromUriString(googleApiConfig.getMapsApiBaseUrl() + "directions/json");
    uriBuilder.queryParam("origin", URLEncoder.encode(request.getOrigin(), StandardCharsets.UTF_8));
    uriBuilder.queryParam(
        "destination", URLEncoder.encode(request.getDestination(), StandardCharsets.UTF_8));
    uriBuilder.queryParam("key", googleApiConfig.getGoogleApiKey());
    if (request.getMode() != null && !request.getMode().isEmpty())
      uriBuilder.queryParam("mode", request.getMode());
    if (request.getLanguage() != null && !request.getLanguage().isEmpty())
      uriBuilder.queryParam("language", request.getLanguage());
  ...
  /**
   * 7. Distance Matrix (Maps Distance Matrix API) 本地端為 POST 請求，但 Google API 實際仍是 GET 請求，參數透過 URL
   * Query 傳遞。
   *
  ...
  public JsonNode getDistanceMatrix(DistanceMatrixRequest request) {
    UriComponentsBuilder uriBuilder =
        UriComponentsBuilder.fromUriString(
            googleApiConfig.getMapsApiBaseUrl() + "distancematrix/json");
    uriBuilder.queryParam(
        "origins", URLEncoder.encode(request.getOrigins(), StandardCharsets.UTF_8));
    uriBuilder.queryParam(
        "destinations", URLEncoder.encode(request.getDestinations(), StandardCharsets.UTF_8));
    uriBuilder.queryParam("key", googleApiConfig.getGoogleApiKey());
    if (request.getMode() != null && !request.getMode().isEmpty())
      uriBuilder.queryParam("mode", request.getMode());
    if (request.getLanguage() != null && !request.getLanguage().isEmpty())
