...
public class TravelService {
    ...
    public TravelMain updateTravelMain(TravelMainRequest request) {
        ...
        // 找出現有的 TravelMain
        TravelMain existingTravelMain = travelMainRepository.findById(request.getId())
                .orElseThrow(() -> new NoSuchElementException("找不到 ID 為 " + request.getId() + " 的行程主表。"));
        ...
        if (!oldStartDate.equals(request.getStartDate()) || !oldEndDate.equals(request.getEndDate())) {
            handleTravelDatesUpdate(request.getId(), oldStartDate, oldEndDate, request.getStartDate(), request.getEndDate(), request.getCreatedBy());
        }
    ...
    public TravelDate addTravelDate(UUID travelMainId, LocalDate baseDate, UUID createdBy) {
        ...
        TravelMain travelMain = travelMainRepository.findById(travelMainId)
                .orElseThrow(() -> new NoSuchElementException("找不到行程主表 ID 為 " + travelMainId + " 的記錄。"));
        
        if (null != travelMain.getEndDate() && savedTravelDate.getTravelDate().isAfter(travelMain.getEndDate())) {
    ...
    public void deleteTravelDate(UUID travelDateId) {
        ...
        List<TravelDate> remainingTravelDatesBeforeDeletion = travelDateRepository.findByTravelMainId(travelMainId);

        // 防呆：檢查是否為最後一個 travel_date，如果是，則不允許刪除
        if (remainingTravelDatesBeforeDeletion.size() == 1 && remainingTravelDatesBeforeDeletion.get(0).getId().equals(travelDateId)) {
        ...
        List<TravelDate> remainingTravelDatesAfterDeletion = travelDateRepository.findByTravelMainId(travelMainId);

        // 嚴格連續：重新排序並更新所有剩餘 TravelDate 的日期
        // 找到剩餘日期中的最早日期作為新的起始點
        LocalDate newStartDate = remainingTravelDatesAfterDeletion.stream()
        ...
        Collections.sort(remainingTravelDatesAfterDeletion, Comparator.comparing(TravelDate::getTravelDate));
     ...
     */
    private void handleTravelDatesUpdate(UUID travelMainId, LocalDate oldStartDate, LocalDate oldEndDate,
                                         LocalDate newStartDate, LocalDate newEndDate, UUID updatedBy) {
        // 獲取所有現有的 travel_date 記錄
        List<TravelDate> existingTravelDates = travelDateRepository.findByTravelMainId(travelMainId);
        Set<LocalDate> existingDatesSet = existingTravelDates.stream()
                .map(TravelDate::getTravelDate)
                .collect(Collectors.toSet());
        // 將現有日期映射到其ID，以便於刪除時找到對應的ID
        java.util.Map<LocalDate, UUID> existingDateIdMap = existingTravelDates.stream()
    ...
    public TravelDetail createTravelDetail(TravelDetailRequest request) {
        ...
        travelDetail.setGoogleMapInfo(request.getGoogleMapInfo());
    ...
    public TravelDetail updateTravelDetail(TravelDetailRequest request) {
        ...
        // 找出現有的 TravelDetail
        TravelDetail existingTravelDetail = travelDetailRepository.findById(request.getId())
                .orElseThrow(() -> new NoSuchElementException("找不到 ID 為 " + request.getId() + " 的行程詳情。"));
